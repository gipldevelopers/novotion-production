...
function LoginForm() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      const res = await fetch(`/api/auth/google/url?callbackUrl=${encodeURIComponent(callbackUrl)}`);
  ...
  return (
    <main className="min-h-screen grid lg:grid-cols-2 bg-white font-sans">
      ...
      <div className="flex flex-col items-center justify-center p-4 lg:p-8 relative bg-slate-50/30 overflow-y-auto">
        <div className="w-full max-w-sm space-y-4 bg-white p-6 rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100">
          ...
          {formError && (
            <div className="bg-red-50 border border-red-100 text-red-600 p-2 rounded-xl flex items-center gap-2 animate-in fade-in slide-in-from-top-1">
              <AlertCircle className="h-3.5 w-3.5 shrink-0" />
              <p className="text-[10px] font-semibold">{formError}</p>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="••••••••"
                  className="w-full h-10 px-4 rounded-xl border border-slate-200 bg-slate-50/50 focus:bg-white focus:border-blue-600 focus:ring-4 focus:ring-blue-100 transition-all outline-none text-sm pr-11"
                  required
                />
                <button
                  type="button"
            ...
            <Button
              type="submit"
              disabled={loading}
              className="w-full h-10 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg shadow-blue-200 transition-all uppercase tracking-widest text-[10px]"
            >
              {loading ? (
          ...
          <Button
            ...
            variant="outline"
            className="w-full h-10 rounded-xl border-slate-200 hover:bg-slate-50 transition-all flex items-center justify-center gap-3 text-slate-700 font-bold text-[10px] uppercase tracking-widest"
          >
            {googleLoading ? (
              <Loader2 className="h-4 w-4 animate-spin" />
            ) : (
          ...
          >
            <Button
              variant="outline"
              className="w-full h-10 rounded-xl text-slate-700 font-bold border-slate-200 hover:bg-slate-50 transition-colors uppercase tracking-widest text-[9px]"
            >
              Create New Account
            </Button>
          </Link>
